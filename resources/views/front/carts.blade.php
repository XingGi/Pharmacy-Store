  ...
  <body>
    ...
    <section class="wrapper flex flex-col gap-2.5">
      ...
      <div class="flex flex-col gap-4" id="itemsList">
        ...
        @empty
        <p>
          Belum terdapat transaksi tersedia.
        </p>
        @endforelse
      ...
      <form enctype="multipart/form-data" action="{{ route('product_transactions.store') }}" method="POST" class="p-6 bg-white rounded-3xl" id="deliveryForm">
        ...
        <div class="flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-2.5">
            ...
            <input type="text" name="address" id="address__" style="background-image: url({{ asset('assets/svgs/ic-location.svg') }})"
              class="form-input" value="Tedjamudita 3">
          </div>
          <!-- City -->
          <div class="flex flex-col gap-2.5">
            <label for="city" class="text-base font-semibold">City</label>
            <input type="text" name="city" id="city__" style="background-image: url({{ asset('assets/svgs/ic-map.svg') }})" class="form-input"
              value="Bolavia">
          </div>
          <!-- Post Code -->
          <div class="flex flex-col gap-2.5">
            <label for="postcode" class="text-base font-semibold">Post Code</label>
            <input type="number" name="post_code" id="postcode__" style="background-image: url({{ asset('assets/svgs/ic-house.svg') }})"
              class="form-input" value="22081882">
          </div>
          <!-- Phone Number -->
          <div class="flex flex-col gap-2.5">
            <label for="phonenumber" class="text-base font-semibold">Phone Number</label>
            <input type="number" name="phone_number" id="phonenumber__" style="background-image: url({{ asset('assets/svgs/ic-phone.svg') }})"
          ...
          <div class="flex flex-col gap-2.5">
            ...
            <input type="file" name="proof" id="proof_of_payment__" style="background-image: url({{ asset('assets/svgs/ic-folder-add.svg') }})"
              class="form-input">
          </div>
        </div>
        </div>
    ...
    <script>
      function calculatePrice(){
        ...
        document.getElementById('checkout-delivery-fee').textContent = 'Rp ' + deliveryFee.toLocaleString('id', {minimumFractionDigits: 2, maximumFractionDigit: 2});

        document.getElementById('checkout-sub-total').textContent = 'Rp ' + subTotal.toLocaleString('id', {minimumFractionDigits: 2, maximumFractionDigit: 2});

        const tax = 11 * subTotal / 100;
        document.getElementById('checkout-ppn').textContent = 'Rp ' + tax.toLocaleString('id', {minimumFractionDigits: 2, maximumFractionDigit: 2});

        const insurance = 23 * subTotal / 100;
        document.getElementById('checkout-insurance').textContent = 'Rp ' + insurance.toLocaleString('id', {minimumFractionDigits: 2, maximumFractionDigit: 2});

        const grandTotalPrice = subTotal + tax + insurance + deliveryFee;
        document.getElementById('checkout-grand-total').textContent = 'Rp ' + grandTotalPrice.toLocaleString('id', {minimumFractionDigits: 2, maximumFractionDigit: 2});

        document.getElementById('checkout-grand-total-price').textContent = 'Rp ' + grandTotalPrice.toLocaleString('id', {minimumFractionDigits: 0, maximumFractionDigit: 2});
